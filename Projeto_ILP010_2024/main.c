...
main(int argc, char *argv[])
{
    ...
    int numero_conta_pesquisada = 0, resp_menu_gerente = 0,
        menu_movimentacao = 0, clientes_cadastrados = verifica_cadastrados(),
        conta_buscada, indice_encontrado, conta_encontrada_inicio = 0,
        senha_conta = 0;
    ...
    while (numero_conta_pesquisada == 0) {
        ...
        if (numero_conta_pesquisada == 9) {
            ...
            for (int i = 0; i < clientes_cadastrados; i++) {
                fprintf(file_agencia, "%d %s %s %d %d %.2lf %d\n",
                        ...
                        clientes_agencia[i].saldo_atual,
                        clientes_agencia[i].chave_pix);
            }
            sleep(2);
            return 0;
        }

        conta_encontrada_inicio = encontrar_conta(clientes_agencia,
                                                  numero_conta_pesquisada, 10);
        if (conta_encontrada_inicio == 20) {
            ...
            if (senha_conta !=
                ...
                if (clientes_agencia[conta_encontrada_inicio].conta_corrente ==
                    1234) {
                    printf(
                        "Você está prestes a entrar no banco. Por favor, "
                        "aguarde... "
                        "\n\n");
                    sleep(3);
                    resp_menu_gerente = 0;
                    while (resp_menu_gerente == 0) {
                        ...
                        switch (resp_menu_gerente) {
                            ...
                            case 1:
                                printf(
                                    "\nDigite o número da conta a ser "
                                    "movimentada: ");
                                scanf("%d", &conta_buscada);
                                indice_encontrado = encontrar_conta(
                                    clientes_agencia, conta_buscada, 10);
                                if (indice_encontrado != 20) {
                                    printf(
                                        "Conta encontrada em nosso "
                                        "sistema\nAguarde a leitura "
                                        "dos dados\n\n");
                                    sleep(3);
                                    menu_movimentacao = movimentar_conta(
                                        clientes_agencia, indice_encontrado,
                                        10);
                                    if (menu_movimentacao == 1) {
                                        resp_menu_gerente = 0;
                                    } else {
                                        printf(
                                            "Pane no sistema,  alguma coisa  "
                                            "está desco n figura "
                                            "da\nSaindo do programa com "
                                            "segurança...");
                                        sleep(4);
                                        return 0;
                                    }
                                    ...
                                    printf(
                                        "Conta não encontrada, por favor, "
                                        "tente novamente\n\n");
                                    resp_menu_gerente = 0;
                                }
                                break;
                            ...
                            case 2:
                                cria_conta_cliente(clientes_agencia,
                    ...
                    menu_movimentacao = movimentar_conta(
                        clientes_agencia, conta_encontrada_inicio, 10);
                    if (menu_movimentacao == 1) {
                        numero_conta_pesquisada = 0;
                        continue;
                    } else {
                        printf(
                            "Pane no sistema, alguma coisa está "
                            "desconfigurada\nSaindo "
