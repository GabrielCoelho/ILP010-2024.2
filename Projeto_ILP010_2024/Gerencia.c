...
exibe_menu_gerente()
{
    ...
    printf("----------------------------------------\n\n\n");
    printf(
        "Movimentação de conta\t\tAbrir nova Conta\t\tExibir Saldo\n   - "
...
saque_deposito_conta(int operacao, Cliente c[], int indice_da_conta)
{
    ...
    if (valor_saque_dep > c[indice_da_conta].saldo_atual && operacao == 1) {
        printf(
            "Quantia desejada maior que saldo disponível, por favor, "
        ...
        sleep(2);
        if (operacao == 1) {
            imprime_relatorio(1, mensagem_operacao,
                              c[indice_da_conta].conta_corrente, 0,
                              c[indice_da_conta].saldo_atual,
                              c[indice_da_conta].saldo_atual - valor_saque_dep,
                              valor_saque_dep);
            c[indice_da_conta].saldo_atual = c[indice_da_conta].saldo_atual -
                                             valor_saque_dep;
        } else {
            imprime_relatorio(2, mensagem_operacao,
                              c[indice_da_conta].conta_corrente, 0,
                              c[indice_da_conta].saldo_atual,
                              c[indice_da_conta].saldo_atual + valor_saque_dep,
                              valor_saque_dep);
...
transferencia_entre_contas(Cliente c[], int indice_da_conta)
{
    ...
    int conta_destino;
    printf(
        "A taxa de transferência entre contas é de R$ 22.50\n\nDigite a "
    ...
    if (valor_transferencia > c[indice_da_conta].saldo_atual - 22.50) {
        ...
        if (indice_conta_destino == 20) {
            ...
            sleep(2);
            imprime_relatorio(
                4, "Transf.", c[indice_da_conta].conta_corrente,
            ...
            c[indice_da_conta].saldo_atual = c[indice_da_conta].saldo_atual -
                                             valor_transferencia - 22.50;
            c[indice_conta_destino].saldo_atual = c[indice_conta_destino]
                                                      .saldo_atual +
                                                  valor_transferencia;
        }
        return 0;
...
pix_entre_contas(Cliente c[], int indice_da_conta)
{
    ...
    if (valor_pix > c[indice_da_conta].saldo_atual) {
        ...
        if (indice_pix_destino == 20) {
            printf(
                "Não existe nenhuma chave PIX como a informada! "
                "\nRetornando ao "
                "menu gerencial");
            return 0;
        } else {
            ...
            c[indice_da_conta].saldo_atual = c[indice_da_conta].saldo_atual -
                                             valor_pix;
            c[indice_pix_destino]
                .saldo_atual = c[indice_pix_destino].saldo_atual + valor_pix;
        }
        return 0;
    }
}
...
movimentar_conta(Cliente c[], int indice_da_conta, int tamanho_agencia)
{
    ...
    while (opcao == 0) {
        ...
        printf("----------------------------------------\n\n\n");
        printf(
            "Selecione a partir do menu abaixo: \n1. Saque\t2. "
            "Depósito\n3. "
            "Pix\t\t4. Transferência\n9. Voltar ao início\n");
        scanf("%d", &opcao);
        switch (opcao) {
            ...
            case 4:
                ...
                break;
            case 9:
                printf("Retornando com segurança...\n");
                sleep(2);
                return 1;
        }
    }
...
cria_conta_cliente(Cliente *c, int tamanho_agencia)
{
    ...
    if (tamanho_agencia == 10) {
        ...
        printf(
            "Muito bem, precisaremos de alguns dados do cliente\nPor "
            "favor, "
        ...
        while (verifica_conta == 0) {
            ...
            if (encontrar_conta(c, verifica_conta, tamanho_agencia) == 20) {
                ...
                if (encontrar_conta(c, verifica_conta + 123, tamanho_agencia) ==
                    20) {
        ...
        printf(
            "Agora iremos criar a chave pix para esta conta!\nO "
            "padrão são "
        ...
        while (chave_pix == 0) {
            ...
            if (encontrar_pix(c, chave_pix, tamanho_agencia) == 20) {
                ...
                if (encontrar_pix(c, chave_pix + 130, tamanho_agencia) == 20) {
                    ...
                    printf(
                        "Chave PIX já consta no sistema e "
 ...
 */
void
imprime_relatorio(int operacao, char s[11], int conta, int conta_destino,
